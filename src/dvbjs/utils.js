...
export function WGS84toGK4(lng, lat) {
  return proj4("WGS84", "GK4", [lng, lat]).map(Math.round);
}

export function WGS84toWm(lng, lat) {
  return proj4("WGS84", "WM", [lng, lat]).map(Math.round);
}

export function WmOrGK4toWGS84(lng, lat) {
...
export function convertCoordinates(s) {
  const coords = [];
...
export function parseDate(d) {
  ...
  return new Date(parseInt(matches));
}

export function parseDiva(d) {
  return d && d.Number
...
export function parseConnections(data) {
  let connections = [];
...
export function extractTrip(trip) {
  const nodes = trip.PartialRoutes.map((node) =>
